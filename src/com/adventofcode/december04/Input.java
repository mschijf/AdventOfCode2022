...
public class Input {
    ...
    public List<Integer> getBingoNumbers() {
        return split(inputList.get(0), ",").stream().map(Integer::parseInt).collect(Collectors.toList());
    }

    public List<BingoCard> getBingoCards() {
        List<BingoCard> bingoCardList = new ArrayList<>();
        for (int lineNumber = 2; lineNumber  < inputList.size(); lineNumber += 6) {
            bingoCardList.add(new BingoCard(inputList.subList(lineNumber, lineNumber+5)));
        }
